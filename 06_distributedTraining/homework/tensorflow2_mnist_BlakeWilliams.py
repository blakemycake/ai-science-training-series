...
import time
t0 = time.time()
parser = argparse.ArgumentParser(description='TensorFlow MNIST Horovod HW')
...
#HVD 4 - optimize learning rate
opt = tf.optimizers.Adam(learning_rate=0.00005*hvd.size())
...
# Restore model
#latest_checkpoint = tf.train_latest_checkpoint(checkpoint_dir)
...
#metrics['mean_train_acc'] = []
#metrics['mean_test_acc'] = []
for ep in range(args.epochs):
    ...
    for batch, (images, labels) in enumerate(dataset.take(nstep)):
        ...
        # HVD 5 - broadcast model and parameters from rank 0
        if (nstep==0 and ep==0 and hvd.rank()==0):
            hvd.broadcast_variables(mnist_model.trainable_variables, root_rank=0)
            hvd.broadcast_variables(opt.variables(), root_rank=0)            
        
        train_loss += loss_value/nstep
        train_acc += acc/nstep

        # HVD 6 - checkpointing for rank 0
        if (batch % 100 ==0) and (hvd.rank()==0): 
            checkpoint.save(checkpoint_dir)
            print('Epoch - %d, step #%06d/%06d\tLoss: %.6f' % (ep, batch, nstep, loss_value))
        
        
        #if mean_train_accuracy is None:
        #    mean_train_accuracy = train_acc
        #else:
        #    mean_train_accuracy += train_acc
        end = time.time()    
    #mean_train_accuracy /= nstep        
    # HVD-8 average the metrics
    #mean_train_accuracy = hvd.allreduce(mean_train_accuracy, average=True)
    #if (hvd.rank()==0):
    #    print(f"Training accuracy after epoch {ep}: {mean_train_accuracy:.4f}.")
    ...
    for batch, (images, labels) in enumerate(test_dset.take(ntest_step)):
        ...
        tt1 = time.time()
        print('E[%d], train Loss: %.6f, training Acc: %.3f, val loss: %.3f, val Acc: %.3f\t Time: %.3f seconds' % (ep, train_loss, train_acc, test_loss, test_acc, tt1 - tt0))

        #val_loss = total_test_loss
        #val_acc = total_test_acc

    
    #mean_test_accuracy /= nstep        
    # HVD-8 average the metrics
    #mean_test_accuracy = hvd.allreduce(mean_test_accuracy, average=True)
    #if (hvd.rank()==0):
    #    print(f"Validation accuracy after epoch {ep}: {mean_test_accuracy:.4f}.")
            
    metrics['train_acc'].append(train_acc.numpy())
    metrics['train_loss'].append(train_loss.numpy())
